...
`git clone https://github.com/ByteLegendQuest/java-fix-factorial`

A project directory named `java-fix-factorial` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-factorial` directory which is created just now.
